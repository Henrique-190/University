...
%
\just\equiv{ Fusão-+, Universal-+ e Natural-+ }
%
  |either (outExpAr . (const X)) (outExpAr . either N (either bin (uncurry Un))) = i1 . i2|
%
\just\equiv{ Eq-+ }
%
\left\{
...
\centerline{\xymatrix{
   ExprAr A \ar[d]_-{|f|}
                \ar@@/^2pc/ [rr]^-{|outExprAr|} & \qquad \cong
&   1 + (A + (BinOp \times (ExprAr A \times ExprAr A) + UnOp \times ExpAr A))  \ar[d]^{|id + (id + (id >< (f >< f) + id >< f))|} 
                                     \ar@@/^2pc/ [ll]^-{|inExprAr|}
\\
    |a| &  & 1 + (A + (BinOp \times (a \times a) + UnOp \times a))\ar[ll]
}}
...
%
\just\equiv{\textcolor{blue}{Aplicando\ a\ definição\ dada\ de\ baseExprAr}}
%
...
\newline
\textbf{g\_eval\_exp}
\par\noindent\hspace{0.5cm}Por forma a obtermos o valor final de uma expressão, podemos recorrer ao uso do catamorfismo de \textit{ExprAr}, definindo o diagrama de \textit{eval\_exp} da seguinte maneira.
\begin{eqnarray*}
\xymatrixcolsep{0.5pc}\xymatrixrowsep{5pc}
\centerline{\xymatrix{
...
\end{eqnarray*}
Como cada coproduto corresponde a um either, então através do diagrama podesmos chegar ao seguinte gene do catamorfismo eval\_exp:
\begin{code}
g_eval_exp a = either (g1 a) g2 where
  g1 a = const a
  g2 = either g3 g4 where
    g3 = id 
    g4 = either g5 g6 where
      g5(Sum ,(a,b)) = a+b
      g5(Product, (a,b)) = a*b
      g6(Negate, a) = -a 
      g6(E, a) = expd a
...
\newline \newline
Reparamos que as resras matemáticas aqui só se aplicavam ao Product e o resto decidimos manter parecisso á função outExpAr, para que a estrutura de dados do anamorfismo se mantivesse. Assim, isto permitiu que o catamorfismo se mantivesse igual ao anterio.
\begin{code}
clean X = i1()
clean (N a) = i2(i1 a)
clean (Bin op a b) = if op==Product && (a == N 0 || b == N 0) then i2(i1 0) else i2(i2(i1 (op,(a,b))))
...
sd_gen :: Floating a =>
    ...
    -> (ExpAr a, ExpAr a)
sd_gen = either g1 g2 where
  g1 = const(X, N 1)
  g2 = either g3 g4 where
    g3 a = (N a, N 0)
    g4 = either  g5 g6 where
      g5(Sum, ((ex1, ex2),(ex3,ex4))) = (Bin Sum ex1 ex3, Bin Sum ex2 ex4)
      g5(Product, ((ex1,ex2),(ex3,ex4))) = 
        (Bin Product ex1 ex3, Bin Sum (Bin Product ex1 ex4) (Bin Product ex2 ex3))
      g6(Negate, (ex1,ex2)) = (Un Negate ex1, Un Negate ex2)
      g6(E , (ex1, ex2)) = (Un E ex1, Bin Product (Un E ex1) ex2)
\end{code}
---
\begin{code}
ad_gen a = either (g1 a) g2 where
  g1 a = const (a, 1)
  g2 = either g3 g4 where
    g3 a = (a, 0)
    g4 = either g5 g6 where
      g5(Sum, ((ex1, ex2),(ex3,ex4))) = (ex1 + ex3, ex2 + ex4)
      g5(Product, ((ex1,ex2), (ex3,ex4))) = (ex1*ex3, ex1*ex4 + ex2*ex3)
      g6(Negate, (ex1,ex2)) = (negate ex1, negate ex2)
      g6(E, (ex1,ex2)) = (expd ex1, ex2 * (expd ex1)) 
\end{code}
...
Seja C(0) = 1 e C(n+1) =
\begin{eqnarray*}
	\frac{2*(2n+1)}{n+2}(f(x)+g(x))=\frac{d}{dx}(f(x))+\frac{d}{dx}(g(x))
\end{eqnarray*}
Seja C(0) = 1 e 
\begin{eqnarray*}
	C(n+1) = \frac{2*(2n+1)}{n+2}
\end{eqnarray*}
...
\left\{
   \begin{array}{lll}
 |catF 0 = 1;\\
 |catF (n+1) = (cat F n) * (cat S n)/(cat T n);\\
 |catS 0 = 2;\\
 |catS (n+1) = (cat S n) + 2;\\
 |catT 0 = 2;\\
 |catT (n+1) = (cat F n) + 2*n + 2;\\
  \end{array}
\right.
%
...
loop(cF, cS, cT) = (div (cF * cS) cT, cS + 4, cT + 1) 
inic = (1,2,2)
prj(cF, cS, cT) = cF
\end{code}
por forma a que
...
É-nos dito no enunciado que |avg_aux = cata (either b q)| = |split avg length|, então,
o raciocínio é partir de |either b q| e tentar obter um split que permita ter
um catamorfismo de um split passível de aplicação da regra Fokkinga. Assim:
...
Tendo um catamofismo de um split, aplique-se Fokkinga:

\begin{eqnarray}
\start
  |split (either (p1 . b) (p1 . q)) (either (p2 . b) (p2 . q)) = split avg length|
%
\just\equiv{ Fokkinga }
%
  |lcbr(
...
%
  |lcbr(
        avg.(either nil cons)= (either (p1.b) (p1.q)).(id + id >< (split avg length))
    )(
        length.(either nil cons)= (either (p2.b) (p2.q)).(id + id >< (split avg length))
    )|
%
\just\equiv{ fusão-+ }
%
|lcbr(
        either (avg. nil) (avg . cons) = (either (p1.b) (p1.q)).(id + id >< (split avg length))
    )(
        either (length . nill) (len . cons) = (either (p2.b) (p2.q)).(id + id >< (split avg length))
    )|
%
\just\equiv{ Def-x }
%
\left\{
   \begin{array}{lll}
      |either (avg. nil) (avg . cons) = (either (p1.b) (p1.q)). (id + (split (id . p1) ((split avg len) . p2)))|\\
      |either (length . nill) (len . cons) = either (p1.b) (p1.q)). (id + (split (id . p1) ((split avg len) . p2)))|\\
    \end{array}
\right.
%
\just\equiv{ Absorção-+ }
%
|lcbr(
        either (avg. nil) (avg . cons) = either (p1 . b . id) (p1 . q . (split avg len) . p2)
    )(
        either (length . nill) (len . cons) = either (p2 . b . id) (p2 . q . (split avg len) . p2)
    )|
%
...
\left\{
   \begin{array}{lll}
    |(avg. nil) = (p1 . b)|\\
    |(avg . cons) = (p1 . q . (split avg len) . p2)|\\
    |(length . nill) = (p2 . b . id)|\\
    |(len . cons) = (p2 . q . (split avg len) . p2)|\\
    \end{array}
\right.
%
...
\left\{
   \begin{array}{lll}
      |avg (nil l) = p1 (b l)|\\
      |avg (cons (a,l)) = p1 (q (a,(avg l , length l)))|\\
      |length (nil l) = p2 (b l)|\\
      |length (cons (a,l)) = p2 (q (a,(avg l , length l)))|\\
  \end{array}
\right.
%
\just\equiv{ Def nil, Def cons, avg e length [] = 0 }
%
\left\{
   \begin{array}{lll}
      |p1 b = 0|\\
      |avg (a:l) = p1 (q (a,(avg l, length l))|\\
      |p2 b = 0|\\
      |length (a:l) = p2 (q (a,(avg l, length  l))|
  \end{array}
\right.
\qed
\end{eqnarray}

Como se compreende, com |p1 b = 0|, qualquer que seja o b, o resultado é sempre 0, logo |b = (const b)|, ou seja, b é uma função constante, tendo-se como solução:
...
Logo, tem-se a solução:
\begin{code}
avg_aux = cataList (either (const b) q) where
  b = (0,0)
  q (c,(avg,len)) = ( (avg * len + c) / (succ len )  , succ len)  
\end{code}

Confrontando com esta solução com o obtido anteriormente, percebe-se também que o |const b| é o inicializador do ciclo \textit{for} (|const b = (0,0)|) e o loop (b) é definido por |Def avg| dada pelo enunciado e |Def length| de uma lista.

Solução para árvores de tipo \LTree:

Esta demonstração, em termos de leis é extremamente parecida com a anterior.

Através das expressões obtidas após a aplicação de Fokkinga, temos:

\begin{eqnarray*}
\start
    |lcbr(
        avg . in = (either (p1.(const b)) (p1.q)) . F (split avg length)
    )(
        len . in = (either (p2.(const b)) (p2.q)) . F (split avg length)
    )|
%
\just\equiv{ Def in , Def F}
%
  |lcbr(
        avg . [Leaf,Fork] = (either (p1.(const b)) (p1.q)) . (id + (split avg length >< split avg length))
    )(
        len . [Leaf,Fork] = (either (p2.(const b)) (p2.q)) . (id + (split avg length >< split avg length))
    )|
%
\just\equiv{ Fusão-+; Def-x; Absorção-+; Eq-+; Natural-id; Igualdade Extensional; Def split; Def-comp}
%
\left\{
   \begin{array}{lll}
      |avg (Leaf l) = p1 . (const b) l|\\
      |avg (Fork (esq,dir)) = p1 (t ((avg esq,length esq), (avg dir, length dir)))|\\
      |length (Leaf l) = p2 . (const b) l|\\
      |length (Fork (esq,dir)) = p2 (t ((avg esq,length esq), (avg dir, length dir)))|\\
  \end{array}
\right.
%
\just\equiv{ Def avg ; length leaf l = (l,1) }
%
\left\{
   \begin{array}{lll}
      |l = p1 b|\\
...
Conclui-se, então |length Fork = length (SubRightTree) + length (SubLeftTree)| 

\begin{code}
avgLTree = p1.cataLTree gene where
   gene = either b t
   b a = (a,1)
   t ((esq1,esq2),(dir1,dir2)) = ((esq1*esq2 + dir1*dir2) / (esq2 + dir2), esq2+dir2) 
\end{code}
